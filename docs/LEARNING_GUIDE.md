...
1. The guest picks **"On arrival"** or **"Pay online now"** right on the booking form (`payment_preference`, stored with the booking). When `updateBookingStatus` confirms a booking, it only calls `buildCheckoutSession` (`src/lib/actions/payments.ts`) if that preference is `'upfront'` — a guest who chose "on arrival" gets confirmed with no payment link at all, matching what they actually asked for. Either way, an admin can still generate a link manually from `/admin/bookings` via `PaymentLinkButton`, regardless of the guest's original choice. `buildCheckoutSession` asks Stripe to create a **Checkout Session** for that exact booking's amount — stamped with `metadata: { booking_id }` so the booking can be identified later. Stripe returns a hosted URL; that URL goes into the guest's confirmation email as a "Pay online (test mode)" button.
...
Most admin pages are **dynamically rendered** on every request, because Next.js infers this automatically once a page reads cookies (which `createClient()` does internally to check the session) — there's nothing to cache there, since only the logged-in admin ever sees them. `revalidatePath` is kept in anyway as a correctness guarantee for those routes, not a workaround for a caching bug.

The two public property pages (`/the-elmstead`, `/the-kiln-house`) are a deliberate exception, and worth understanding because getting them cached took two separate changes, not one:

1. **`src/lib/queries/properties.ts`** wraps its Supabase calls in `unstable_cache()`. This matters because only plain `fetch()` calls get Next.js's automatic caching for free — the Supabase SDK doesn't use `fetch()` under the hood, so without this wrapper, every property lookup would hit the database on every request no matter what. These queries also use a new cookie-free `createPublicClient()` (`src/lib/supabase/server.ts`) instead of the usual `createClient()` — calling `cookies()` at all, even just to check "is anyone logged in," forces the *entire* route to render dynamically, which would defeat the point.
2. **`[slug]/page.tsx` and `layout.tsx`** export `generateStaticParams()` and `revalidate = 60`. A dynamic route segment like `[slug]` needs `generateStaticParams()` specifically to be *eligible* for static generation at all — without it, Next.js has no way to know `the-elmstead` and `the-kiln-house` are the only two values worth pre-building, and keeps the route dynamic regardless of any `revalidate` setting.

Together these took both property pages from server-rendering on every single visit to serving a cached static page that only regenerates at most once a minute (confirmed in `npm run build`'s output: `●` next to both routes, "Revalidate 1m"). This came out of a real, measured problem — the site felt slow, and cold serverless starts plus a per-request database round trip were a big part of why.
...
- **Resend** — chosen specifically because Supabase's own built-in email sending turned out to be unreliable for delivery (this was a real, debugged issue early in the project — see the git history around `20260822...`). Resend handles every outbound email: booking notifications, guest receipts, and password reset codes. One real limitation worth knowing: without a **verified sending domain**, Resend only allows delivery to the account's own address — confirmed directly via the API (`"You can only send testing emails to your own email address"`). Admin notifications (to that same account address) work fine today; guest-facing emails silently fail until a domain is verified. DNS records for `amallab.co.uk` have been added on Resend's side, but as of 2026-08-23 this is paused, waiting on Resend's own backend to finish verifying — the fix, once it clears, is a one-line change to the `from` address in `src/lib/email/resend.ts`.
